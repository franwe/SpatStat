...
head(jura)
```
Now, we want our data to be saved as a SpatialPointsDataFrame, so we add coordinates (I keep `jura` just for convenience). Also, we generate a convex grid using the function `build.convex.grid` as a surface for predictions later and ensure that it is of class SpatialPoints:
...
```
For this task, we only consider the concentration of nickel (`Ni`).To get a first impression, we map the measured concentration of nickel with `spplot` and `bubble` (the bubble plot is not very helpful in this display, but I did not want to include very big plots).
...
Now, we construct a map with the locations of the villages and the malaria prevalence using the **leaflet** package. However, this produces html-plots which I don't know how to display here. This is the code I used (which was already provided in the assignment):
```{r, eval=FALSE}
